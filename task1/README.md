...
Такие краевые задачи были выбраны специально. Константа, "плавная" функция и две интересных нелинейных функции.

В ходе экспериментов проводились запуски алгоритма (10 запусков на каждый случай). Результаты его работы представленны в
виде таблиц. Обозначения
...
В зависимости от того, какие значения изначально стоят в узлах сетки, может меняться время работы алгоритма и
вычислительная ошибка. Предполагается, что при умном заполнении сетки, средняя ошибка аппроксимации уменьшится.
...
|------------|--------------------------|--------------------------|--------------------------|--------------------------|
| 0          | $(t: 0.015c) (e: 0.037)$ | $(t: 0.047c) (e: 0.221)$ | $(t: 0.095c) (e: 0.618)$ | $(t: 0.289c) (e: 0.797)$ | 
| average    | $(t: 0.007c) (e: 0)$     | $(t: 0.009c) (e: 0)$     | $(t: 0.011c) (e: 0)$     | $(t: 0.012c) (e: 0)$     | 

#### Результаты запуска для функции $f_2$:

| $net size$ | 20                       | 50                      | 100                      | 200                     | 
|------------|--------------------------|-------------------------|--------------------------|-------------------------|
| 0          | $(t: 0.008c) (e: 0.682)$ | $(t: 0.009c) (e: 0.86)$ | $(t: 0.012c) (e: 0.927)$ | $(t: 0.02c) (e: 0.963)$ | 
| average    | $(t: 0.006c) (e: 0.683)$ | $(t: 0.008c) (e: 0.86)$ | $(t: 0.012c) (e: 0.927)$ | $(t: 0.02c) (e: 0.962)$ |

#### Результаты запуска для функции $f_3$:

| $net size$ | 20                       | 50                      | 100                      | 200                     | 
|------------|--------------------------|-------------------------|--------------------------|-------------------------|
| 0          | $(t: 0.006c) (e: 0.707)$ | $(t: 0.007c) (e: 0.89)$ | $(t: 0.009c) (e: 0.945)$ | $(t: 0.01c) (e: 0.973)$ | 
| average    | $(t: 0.007c) (e: 0.707)$ | $(t: 0.008c) (e: 0.89)$ | $(t: 0.010c) (e: 0.946)$ | $(t: 0.012) (e: 0.973)$ |

#### Результаты запуска для функции $f_4$:

| $net size$ | 20                      | 50                      | 100                      | 200                      | 
|------------|-------------------------|-------------------------|--------------------------|--------------------------|
| 0          | $(t: 0.008c) (e: 0.61)$ | $(t: 0.01c) (e: 0.778)$ | $(t: 0.025c) (e: 0.853)$ | $(t: 0.069c) (e: 0.906)$ | 
| average    | $(t: 0.007c) (e: 1.13)$ | $(t: 0.01c) (e: 0.74)$  | $(t: 0.02c) (e: 0.802)$  | $(t: 0.065c) (e: 0.848)$ | 

Для функции $f_1$, по очевидным причинам, уменьшилось время выполнения, а средняя ошибка аппроксимации стала равна 0.
Для функций $f_2$, $f_3$, $f_4$ время выполнения алгоритма и средняя ошибка аппроксимации не изменились.
Таким образом, от выбора изначального заполнения сетки между 0 и средним значением граничных точек, в общем случае
результат не меняется.
Возможно, существуют другие изначальные заполнения сетки, уменьшающие время выполнения алгоритма и среднюю ошибку.
...
Ожидается, что при уменьшении eps уменьшиться средняя ошибка аппроксимации и увеличится время выполнения алгоритма.
...
обеспечивающий минимальную ошибку. Например, для $f_4$ при $eps = 10^{-6}$ таковым размером сетки является 100.

### Влияение размерности блока на время работы и вычислительную ошибку алгоритма

Ожидается, что для фиксированного размера сетки будет находиться оптимальное значение размера блока. Причем функция времени выполнения алгоритма будет сначала убывать, а потом возрастать.
...
Исходя из полученных результатов можно сказать, что оптимальным размером блока из тех, что использовались для
эксперимента, является — 64.
Величина ошибки при изменении блока остается неизменной.

### Влияние количества потоков на время работы алгоритма

Ожидается, что время выполнения алгоритма уменьшится в несколько раз при запуске на нескольких потоках.
...
Таким образом, при увеличении количества потоков, уменьшается время выполнения алгоритма. Причем, при увеличении
размерности сетки, отношение производительности многопоточного исполнения к однопоточному — увеличивается.
